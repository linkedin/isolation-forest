...
class IsolationForestModelWriteReadTest extends Logging {
  ...
  def isolationForestModelWriteReadTest(): Unit = {
    ...
    val predictedLabels1 = scores1.map(_.predictedLabel).collect
    val predictedLabels2 = scores2.map(_.predictedLabel).collect
  ...
  def isolationForestIdenticalFeatureValuesWriteReadTest(): Unit = {
    ...
    Assert.assertEquals(
      scores1.map(_.outlierScore).collect.toSeq,
      scores2.map(_.outlierScore).collect.toSeq)

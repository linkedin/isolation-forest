 ...
 *
 * This holds one instance, as well as an array of weights which represent the (weighted) number of
 * times which this instance appears in each subsamplingRate. E.g., (datum, [1, 0, 4]) indicates
 * that there are 3 subsamples of the dataset and that this datum has 1 copy, 0 copies, and 4 copies
 * in the 3 subsamples, respectively.
 *
 * @param datum
 *   Data instance.
 * @param subsampleWeights
 *   Weight of this instance in each subsampled dataset.
 */
private[isolationforest] case class BaggedPoint[Datum](
   ...
   * @param numSubsamples
   *   Number of subsamples of this RDD to take.
   * @param randomState
   *   AbstractIntegerDistribution instance used to draw random samples.
   */
  def this(
    datum: Datum,
...
private[isolationforest] case object BaggedPoint {
   ...
   *   Random seed.
   * @return
   *   BaggedPoint dataset representation.
   */
  ...
  /**
   * Convert an input dataset into its BaggedPoint representation, choosing subsamplingRate counts
   * for each instance. This method is a helper for convertToBaggedRDD(). It accepts a randomState
   * parameter of AbstractIntegerDistribution type. Sampling with (without) replacement can be
   * achieved by using a PoissonDistribution (BinomialDistribution) randomState.
   *
   * @param input
   *   Input dataset.
   * @param subsamplingRate
   ...
   * @param seed
   *   Random seed.
   * @param randomStateFactory
   *   A function that accepts a seed and returns a fresh instance of the desired distribution.
   * @return
   *   BaggedPoint dataset representation.
   */
  private def convertToBaggedRDDHelper[Datum](
    ...
    numSubsamples: Int,
    seed: Long,
    randomStateFactory: Long => AbstractIntegerDistribution,
  ): RDD[BaggedPoint[Datum]] =
  ...
  /**
   * Flattens a baggedRDD to a pair RDD. The key is the subsampleIndex. The instances are duplicated
   * in the flattened representation according to their specified subsampleWeight. (e.g.
   * subsampleWeight == 0 then no instances, subsampleWeight == 1 then one instance, subsampleWeight ==
   * 2 then two instances, etc.)
   *
   * @param baggedRdd
   *   BaggedPoint dataset representation.
   * @return
   *   Pair RDD that contains a flattened representation of the data.
   */
  def flattenBaggedRDD[Datum](baggedRdd: RDD[BaggedPoint[Datum]], seed: Long): RDD[(Int, Datum)] = {
     ...
     *
     * @param subsampleWeight
     *   Weight of this instance in each subsampled dataset.
     * @return
     *   The rounded subsampleWeight.
     */
    ...
    baggedRdd.mapPartitionsWithIndex { (partitionIndex, iter) =>
      ...
      iter.flatMap { baggedPoint =>
        baggedPoint.subsampleWeights.zipWithIndex.flatMap { case (subsampleWeight, subsampleId) =>

...
private[isolationforest] case object Nodes {
   ...
   *
   * @param numInstances
   *   The number of data points that terminated in this node during training.
   */
   ...
   *
   * @param leftChild
   *   The left child node produced by splitting the data points at this node.
   * @param rightChild
   *   The right child node by splitting the data points at this node.
   * @param splitAttribute
   *   The feature upon which the split was performed.
   * @param splitValue
   *   The feature value used for the split.
   */
  case class InternalNode(
    ...
    require(
      splitAttribute >= 0,
      "parameter splitAttribute must be >=0, but given invalid value" +
        s" ${splitAttribute}",
    )

...
private[isolationforest] object SharedTrainLogic extends Logging {
  ...
  /**
   * Helper that bags the data (with possible up-sampling), flattens it,
   ...
   * @param numEstimators   Number of trees to train
   * @param bootstrap       Whether to sample with replacement
   * @param randomSeed      Random seed for sampling
  ...
  def createSampledPartitionedDataset(
    ...
    numEstimators: Int,
    bootstrap: Boolean,
    randomSeed: Long): Dataset[DataPoint] = {
    ...
    val flattenedSampledRdd = BaggedPoint.flattenBaggedRDD(sampledRdd, randomSeed + dataset.rdd.getNumPartitions + 1)
  ...
  /**
   * Helper that computes and sets the model threshold based on the current
   * contamination and contaminationError parameters. It's generic for any model that:
   * 1) Has transform() returning a DF with scoreCol
   * 2) Exposes a method to set threshold
   *
   * If contamination == 0.0, the threshold is not set (no outliers). Otherwise, it
   * uses approxQuantile on the training data to match the desired contamination fraction,
   * and logs a warning if the observed contamination differs more than expected.
   *
   * @param model The trained isolation forest model instance.
   * @param data  The training data used to train the model.
   * @param scoreCol The name of the column in the DataFrame that contains the outlier score.
   * @param contamination The user-specified contamination fraction.
   * @param contaminationError The user-specified error tolerance for the contamination fraction.
   */
  def computeAndSetModelThreshold(
    model: { def transform(data: Dataset[_]): DataFrame; def setOutlierScoreThreshold(v: Double): Unit },
    data: Dataset[_],
    scoreCol: String,
    contamination: Double,
    contaminationError: Double): Unit = {
  ...
  /**
   * Helper that trains an isolation tree on each partition of the given Dataset,
   ...
   * @param randomSeed    Random seed for training the trees.
   * @param treeBuilder   A function that builds an isolation tree.
   * @tparam T            A subtype of IsolationTreeBase.
   * @return              An array of T, one per partition.
   */

...
private[isolationforest] case object IsolationForestModelReadWrite extends Logging {
  ...
  class IsolationForestModelReader extends MLReader[IsolationForestModel] with Serializable {
     ...
     *
     * @return
     *   (ensemble metadata, array of root nodes each tree), where the root node is linked with all
     *   descendents
     * @see
    ...
    private def loadImpl(path: String, spark: SparkSession): (Metadata, Array[Node]) = {
      ...
      val metadata =
        loadMetadata(path, spark, "com.linkedin.relevance.isolationforest.IsolationForestModel")
    ...
    /**
     * Builds a binary tree given an array of NodeData instances. The node IDs must have been
     * assigned via pre-order traversal.
     *
     * @param data
     ...
     *
     * @param path
     *   The path to the saved metadata.
     * @param spark
     ...
     *   If non empty, this is checked against the loaded metadata.
     * @return
     *   A [[Metadata]] instance built from the parsed metadata string.
     */
   ...
   *
   * @param instance
   *   The IsolationForestModel instance to write to disk.
   */
  class IsolationForestModelWriter(instance: IsolationForestModel) extends MLWriter {
    ...
    private def saveImplHelper(path: String, spark: SparkSession, extraMetadata: JObject): Unit = {
      ...
      val nodeDataRDD = spark.sparkContext
        .parallelize(instance.isolationTrees.zipWithIndex.toIndexedSeq)

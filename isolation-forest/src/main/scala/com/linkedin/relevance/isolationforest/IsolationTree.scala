 ...
 *
 * @param node
 *   The root node of the isolation tree model.
 */
...
private[isolationforest] case object IsolationTree extends Logging {
  ...
  def fit(data: Array[DataPoint], randomSeed: Long, featureIndices: Array[Int]): IsolationTree = {
    ...
    new IsolationTree(
      generateIsolationTree(data, heightLimit, new Random(randomSeed), featureIndices),
    )
  }
  ...
  def generateIsolationTree(
    ...
    def generateIsolationTreeInternal(
       ...
       *
       * @param data
       *   The data at the particular node in question.
       * @return
       *   Tuple containing the feature index and the split value. Feature index is -1 if no
